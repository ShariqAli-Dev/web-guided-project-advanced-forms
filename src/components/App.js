...
export default function App() {
  ...
  const validate = (name, value) => {
    // let's validate the specific key/value
    yup
      .reach(schema, name)
      // we can then run validate using the value
      .validate(value)
      // if validation is succesful, we can clear the error message
      .then((valid) => {
        setFormErrors({
          ...formErrors,
          [name]: "",
      ...
      /* if the validation is unsuccesful, we can set the error message to the message returned from yup (that we created in our schema) */
      .catch((err) => {
        setFormErrors({
          ...formErrors,
          [name]: err.errors[0],
  ...
  return (
    <div className="container">
      ...
      <FriendForm values={formValues} change={inputChange} submit={formSubmit} disabled={disabled} errors={formErrors} />
